pipeline {
    ...
    environment {
        // Define image names
        BACKEND_IMAGE = "Dinesh19-S/docker-backend:latest"
        FRONTEND_IMAGE = "Dinesh19-S/docker-frontend:latest"
        ...
        stage('Checkout Code') {
            steps {
                withCredentials([usernamePassword(credentialsId: 'github-Dinesh', usernameVariable: 'GIT_USER', passwordVariable: 'GIT_TOKEN')]) {
                    git url: "https://$GIT_USER:$GIT_TOKEN@github.com/Dinesh19-S/kuber.git", branch: 'main'
                }
            }
        }
